  ...
  return (
    <main style={{ minHeight:"100vh", background:"var(--ink)", paddingTop:80 }} dir={isAr ? "rtl" : "ltr"}>
        ...
        <div className="container">
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, marginBottom:16 }}>
            ...
            <span style={{ fontSize:11, fontWeight:700, color:"var(--gold)", letterSpacing:4, textTransform:"uppercase" }}>
              {isAr ? "تعلّم البيانو" : "Learn Piano"}
            </span>
            <div style={{ width:40, height:1, background:"var(--gold)", opacity:0.5 }}/>
          </div>
          <h1 className="font-display" style={{ fontSize:"clamp(40px,7vw,80px)", fontWeight:300, color:"var(--cream)", marginBottom:16, lineHeight:1.1 }}>
            {isAr
              ? <>دروس <span className="gradient-text" style={{fontWeight:800}}>البيانو</span></>
              : <><span className="gradient-text" style={{fontWeight:800}}>Piano</span> Classes</>}
          </h1>
          <p style={{ fontSize:17, color:"var(--text-muted)", maxWidth:580, margin:"0 auto 40px", lineHeight:1.8 }}>
            {isAr
              ? "دروس بيانو فردية مع مدرّس محترف. اختر الباقة التي تناسبك."
              : "One-by-one Piano classes with a professional instructor. Choose the package that fits you."}
          </p>

          <AssessmentCalendar instrument="PIANO" courseName="Piano" />
        </div>
      </section>
      ...
      <div className="container" style={{ paddingTop:64, paddingBottom:80 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:24, maxWidth:1000, margin:"0 auto" }} className="piano-pricing-grid">
          {OPTIONS.map((opt) => (
      ...
      <style>{".piano-pricing-grid { } @media(max-width:850px) { .piano-pricing-grid { grid-template-columns: 1fr !important; } }"}</style>
    </main>
  )
}
