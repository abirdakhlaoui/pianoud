  ...
  return (
    <main style={{ minHeight:"100vh", background:"var(--ink)", paddingTop:80 }} dir={isAr ? "rtl" : "ltr"}>
      ...
      <section style={{ padding:"80px 0 64px", background:"var(--ink)", borderBottom:"1px solid var(--border)", textAlign:"center" }}>
        <div className="container">
          ...
          <h1 className="font-display" style={{ fontSize:"clamp(40px,7vw,80px)", fontWeight:300, color:"var(--cream)", marginBottom:16, lineHeight:1.1 }}>
            {isAr
              ? <>دروس <span className="gradient-text" style={{fontWeight:800}}>العود</span></>
              : <><span className="gradient-text" style={{fontWeight:800}}>Oud</span> Classes</>}
          </h1>
          <p style={{ fontSize:17, color:"var(--text-muted)", maxWidth:580, margin:"0 auto 40px", lineHeight:1.8 }}>
            {isAr
              ? "دروس عود فردية مع مدرّس محترف. اختر الباقة التي تناسبك."
              : "One-by-one Oud classes with a professional instructor. Choose the package that fits you."}
          </p>

          {/* Don't know your level — KEPT */}
          <div style={{ display:"inline-flex", flexDirection:"column", alignItems:"center", gap:12, padding:"36px 56px", borderRadius:20, background:"linear-gradient(135deg, rgba(201,168,76,0.18), rgba(201,168,76,0.06))", border:"2px solid var(--gold)", boxShadow:"0 0 40px rgba(201,168,76,0.25)" }}>
            ...
            <Link href={whatsappAssessmentLink("Oud", isAr)} target="_blank" className="btn-gold" style={{ padding:"14px 40px", fontSize:16, fontWeight:700 }}>
      ...
      <div className="container" style={{ paddingTop:64, paddingBottom:80 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:24, maxWidth:1000, margin:"0 auto" }} className="oud-pricing-grid">
          {OPTIONS.map((opt) => (
            <div key={opt.id} style={{ borderRadius:20, overflow:"hidden", border:"2px solid " + opt.border, background:"var(--ink-card)", display:"flex", flexDirection:"column", position:"relative" }}>
              ...
              <div style={{ padding:"32px 28px", flex:1, display:"flex", flexDirection:"column", background: opt.bg }}>
                ...
                <Link href={whatsappBookingLink("Oud", (isAr ? opt.hours_ar : opt.hours_en), opt.price, isAr)} target="_blank" style={{ marginTop:"auto", display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"14px", borderRadius:10, fontSize:15, fontWeight:700, textDecoration:"none", background:opt.color, color:"#0A0A0A" }}>
      ...
      <style>{".oud-pricing-grid { } @media(max-width:850px) { .oud-pricing-grid { grid-template-columns: 1fr !important; } }"}</style>
    </main>
  )
}
